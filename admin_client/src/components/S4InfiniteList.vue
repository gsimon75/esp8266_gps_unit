...
export default {
    ...
    props: {
        ...
        supplyItems: {
            type: Function,
            default: function (limit, num, before) {
                var from, to;
                if (before) {
                    from = limit.time - num;
                    to = limit.time;
    ...
    methods: {
        ...
        onscroll: function (e) {
            ...
            if (is_near_start && !this.reached_first_item && !this.preload_in_progress) {
                this.preload_in_progress = true;
                let limit_item = (this.items.length > 0) ? this.items[0] : null;
                this.supplyItems(limit_item, this.preload, true).then(new_items => {
                    // this was a backward search so the items are ordered latest-first -> flip
                    new_items.reverse(); 
            ...
            if (is_near_end && !this.reached_last_item && !this.preload_in_progress) {
                ...
                this.supplyItems(limit_item, this.preload, false).then(new_items => {
                    ...
                    let n = this.items.length - this.max_items;
                    if (0 < n) {
